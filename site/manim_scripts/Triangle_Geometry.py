...
class Triangle_Geometry(Scene):
    def construct(self):
        ...
        graphs += Dot(point=grid @ (0, 1, 0), color=YELLOW)
        graphs += Line(start=grid @ ( 0,  1), end=grid @ ( 1, -1), color=RED)
        graphs += Line(start=grid @ ( 1, -1), end=grid @ (-1, -1), color=RED)
        graphs += Line(start=grid @ (-1, -1), end=grid @ ( 0,  1), color=RED)
