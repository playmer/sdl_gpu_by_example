...
pub fn get_specific_content_context(handlebars: &Handlebars<'_>, template_context: &serde_json::Map<String, Value>, inserts: &Vec<(String, String)>, content: &content::Content) -> serde_json::Map<String, Value> {
    ...
    let (current_content, inserts) = {
        ...
        let rendered_content_html = {
            let (rendered_html, toc_items) = markdown::parse_markdown_to_html(&content_title, &content.markdown);
...
pub fn process_content() -> Vec<(PathBuf, String)> {
    ...
    handlebars.register_helper("collapsible-card", Box::new(templates::CollapsibleCardStartHelper{i: Arc::new(RwLock::new(0))}));
    handlebars.register_helper("collapsible-card-end", Box::new(templates::collapsible_card_end_helper));
